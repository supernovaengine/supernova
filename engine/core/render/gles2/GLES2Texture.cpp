...
void GLES2Texture::loadTextureFrameCube(int width, int height, bool depthframe){
    ...
    if (!depthframe){
        ...
        if(!strstr(extensions, "depth_texture")){
            ...
            for (unsigned int i = 0 ; i < 6 ; i++) {
                glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_DEPTH_COMPONENT, width, height, 0, GL_DEPTH_COMPONENT, GL_UNSIGNED_INT, 0);
            }
...
void GLES2Texture::initTextureFrame(int cubeFace){
    glBindFramebuffer(GL_FRAMEBUFFER, frameBuffer);
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, cubeFace, cubeShadowMap, 0);
}
