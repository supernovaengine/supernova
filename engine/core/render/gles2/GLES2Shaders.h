...
"  #ifdef HAS_SHADOWSCUBE\n"
"    bool checkShadowCube(vec3 vPosition, vec3 lightPos, samplerCube shadowMap, vec2 shadowCameraNearFar) {\n"
...
"                   inShadow = checkShadowCube(v_Position, u_PointLightPos[i], u_shadowsMapCube[i], u_shadowCameraNearFar);\n"
"               }else{\n"
"                   for(int j = 0; j < MAXLIGHTS; ++j){\n"
"                       if (u_PointLightShadowIdx[i] == j){\n"
"                           inShadow = checkShadowCube(v_Position, u_PointLightPos[i], u_shadowsMapCube[j], u_shadowCameraNearFar);\n"
"                           break;\n"
"                       }\n"
"                   }\n"
"               }\n"
