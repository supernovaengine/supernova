...
"  uniform int u_DirectionalShadowIdx[MAXLIGHTS];\n"
...
"  #ifdef HAS_SHADOWSCUBE\n"
"    bool checkShadowCube(vec3 vPosition, vec3 lightPos, samplerCube shadowMap, vec2 shadowCameraNearFar, float cosTheta) {\n"
...
"        float lenToLight = length(fragToLight) / shadowCameraNearFar.y;\n"
"        float bias = 0.01*tan(acos(cosTheta));\n"
"        bias = clamp(bias, 0.0, 0.01);\n"
"        if (lenToLight > lenDepthMap + bias){\n"
"            return true;\n"
"        }\n"
"        return false;\n"
"    }\n"
...
"                   inShadow = checkShadowCube(v_Position, u_PointLightPos[i], u_shadowsMapCube[i], u_shadowCameraNearFar, PointLightcosTheta);\n"
"               }else{\n"
"                   for(int j = 0; j < MAXLIGHTS; ++j){\n"
"                       if (u_PointLightShadowIdx[i] == j){\n"
"                           inShadow = checkShadowCube(v_Position, u_PointLightPos[i], u_shadowsMapCube[j], u_shadowCameraNearFar, PointLightcosTheta);\n"
"                           break;\n"
"                       }\n"
"                   }\n"
"               }\n"
...
"                   inShadow = checkShadow(v_ShadowCoordinates[i], u_shadowsMap[i], SpotLightcosTheta);\n"
"               }else{\n"
"                   for(int j = 0; j < MAXLIGHTS; ++j){\n"
"                       if (u_SpotLightShadowIdx[i] == j){\n"
"                           inShadow = checkShadow(v_ShadowCoordinates[j], u_shadowsMap[j], SpotLightcosTheta);\n"
"                           break;\n"
"                       }\n"
"                   }\n"
"               }\n"
...
"                   inShadow = checkShadow(v_ShadowCoordinates[i], u_shadowsMap[i], DirectionalLightcosTheta);\n"
"               }else{\n"
"                   for(int j = 0; j < MAXLIGHTS; ++j){\n"
"                       if (u_DirectionalShadowIdx[i] == j){\n"
"                           inShadow = checkShadow(v_ShadowCoordinates[j], u_shadowsMap[j], DirectionalLightcosTheta);\n"
"                           break;\n"
"                       }\n"
"                   }\n"
"               }\n"
...
"        lightDistance = lightDistance / u_shadowCameraNearFar.y;\n"
