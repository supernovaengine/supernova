...
"    bool checkShadow(vec4 shadowCoordinates, sampler2D shadowMap) {\n"
...
"                   inShadow = checkShadow(v_ShadowCoordinates[i], u_shadowsMap[i]);\n"
"               }else{\n"
"                   for(int j = 0; j < MAXLIGHTS; ++j){\n"
"                       if (u_SpotLightShadowIdx[i] == j){\n"
"                           inShadow = checkShadow(v_ShadowCoordinates[j], u_shadowsMap[j]);\n"
"                           break;\n"
"                       }\n"
"                   }\n"
"               }\n"
